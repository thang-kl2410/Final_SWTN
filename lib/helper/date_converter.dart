...
class DateConverter {
  ...
  static String formatDateLong(int? milliseconds){
    return milliseconds != null ? DateFormat("dd/MM/yyyy").format(DateTime.fromMillisecondsSinceEpoch(milliseconds)) : '-';
  }
  ...
  static String convertIntToDate(String? timestamp) {
    if(timestamp != null){
  ...
  static String convertIntToDateInChat(String? timestamp) {
    if(timestamp != null){
      DateTime dateTime = DateTime.fromMillisecondsSinceEpoch(int.parse(timestamp));
      String formattedDate = "${dateTime.day.toString().padLeft(2, '0')} THG ${dateTime.month.toString().padLeft(2, '0')} LÚC ${dateTime.hour}:${dateTime.minute}";
      return formattedDate;
    }
    return '';
  }
